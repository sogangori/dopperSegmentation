...
modelName = "./DAS_COLOR/weights/trimap_once.pd"
LABEL_SIZE_C = 2
ensemble= 10
depth0 = ensemble
...
conv_l3_weights = tf.get_variable("l3", shape=[3, 3, depth0, 3], initializer =tf.contrib.layers.xavier_initializer())
beta_l3 = tf.Variable(tf.constant(0.0, shape=[3]),trainable=True)
gamma_l3 = tf.Variable(tf.constant(1.0, shape=[3]), trainable=True) 
...
def inference(inData, train,step):
    ...
    in2 = inData = helper.Gaussian_noise_Add(inData, 0.2, 0.5)
