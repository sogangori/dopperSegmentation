...
import Model_trimap_cardinal as model 
...
def main(argv=None):        
  ...
  regular =  helper.regularizer()    
  loss = entropy + 1e-3 * loss_unknown + 1e-5 * regular
  ...
  with tf.Session(config=config) as sess:    
    ...
    for step in xrange(NUM_EPOCHS):
      ...
      for iter in range((int)(test_offset)):
          ...
          if AUGMENT<1:              
              ...
              iou_valid,iou_valid_known = sess.run([mean_iou,mean_iou_known], feed_dict_flip)    
          ...
          if iter % EVAL_FREQUENCY == 0:
            ...
            print('e%d,i%d,tri(%.0f,%.0f,%.0f),L:(%.5f,%.4f),IoU(tr%.0f,k%.0f,va%.1f,k%.1f), lr %.4f' % 
                  (step, iter,back*100,fore*100,unkno*100,entro,l_unknown,
    ...
    iou_valid,iou_valid_known = sess.run([mean_iou,mean_iou_known], feed_dict_valid)
    trimap_mask,unknown_mask,iou_test,iou_test_known = sess.run([trimap,unknown,mean_iou,mean_iou_known], feed_dict= feed_dict_test)
