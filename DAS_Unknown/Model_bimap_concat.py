...
modelName = "./DAS_Unknown/weights/bimap_concat.pd"
LABEL_SIZE_C = 2
ensemble= 12
depth0 = ensemble
...
with tf.variable_scope('bimap'):
    conv_l0_weights  = tf.get_variable("w1", shape=[3, 3, ensemble+3, depth0*2], initializer =tf.contrib.layers.xavier_initializer())
...
def inference(trimap, inData, train,step):
    helper.isDrop = train
    helper.keep_prop = 0.7
       
    inData = helper.Gaussian_noise_Add(inData, 0.2, 0.5)
    trimap = tf.nn.softmax( trimap)
    inData = tf.concat([inData, trimap],3)
