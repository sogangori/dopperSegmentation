...
modelName = "./DAS_Unknown/weights/bimap_small.pd"
LABEL_SIZE_C = 2
ensemble= 12
depth0 = ensemble
...
def inference(inData, train,step):
    ...
    #0
    pool = tf.nn.max_pool(inData,pool_stride2,strides=pool_stride2,padding='SAME')
    ...
    feature0 = pool = helper.conv2dRelu(pool,conv_m0_weights,beta_m0)   

    #2
    pool = tf.nn.max_pool(pool,pool_stride2,strides=pool_stride2,padding='SAME')    
    feature1 = pool = helper.conv2dRelu (pool,conv_s0_weights,beta_s0)   
