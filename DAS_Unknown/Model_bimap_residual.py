...
modelName = "./DAS_Unknown/weights/bimap_res.pd"
LABEL_SIZE_C = 2
ensemble= 12
depth0 = ensemble
...
with tf.variable_scope('bimap'):
    ...
    conv_xx0_weights = tf.get_variable("xx0", shape=[3, 3, depth0*64, depth0*32], initializer =tf.contrib.layers.xavier_initializer())
    beta_xx0 = tf.Variable(tf.constant(0.0, shape=[depth0*32]))
    gamma_xx0 = tf.Variable(tf.constant(1.0, shape=[depth0*32])) 

    conv_x2_weights = tf.get_variable("x2", shape=[3, 3, depth0*32, depth0*32], initializer =tf.contrib.layers.xavier_initializer())
    ...
    conv_l2_weights = tf.get_variable("l2", shape=[3, 3, depth0*2, 3], initializer =tf.contrib.layers.xavier_initializer())
    beta_l2 = tf.Variable(tf.constant(0.0, shape=[3]))
    gamma_l2 = tf.Variable(tf.constant(1.0, shape=[3])) 

    conv_l3_weights = tf.get_variable("l3", shape=[3, 3, 3,depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_l3 = tf.Variable(tf.constant(0.0, shape=[depth0]),trainable=True)
    gamma_l3 = tf.Variable(tf.constant(1.0, shape=[depth0])) 

    conv_l4_weights = tf.get_variable("l4", shape=[3, 3, depth0, 2], initializer =tf.contrib.layers.xavier_initializer())
    beta_l4 = tf.Variable(tf.constant(0.0, shape=[2]),trainable=True)
    gamma_l4 = tf.Variable(tf.constant(1.0, shape=[2])) 

def inference(inData, trimap, train,step):
    helper.isDrop = train
    helper.isTrain = train
    helper.keep_prop = 0.8
       
    inData = helper.Gaussian_noise_Add(inData, 0.1, 0.2)
    ...
    pool = helper.conv2dBN(pool,conv_l2_weights,beta_l2,gamma_l2)
    trimap_resize = tf.nn.softmax(helper.resize(trimap, up_shape[1],up_shape[2]))    
    pool = tf.nn.relu(tf.add(trimap_resize, pool)) 
    pool = helper.conv2dBN_Relu(pool,conv_l3_weights,beta_l3,gamma_l3)
    pool = helper.conv2dBN_Relu(pool,conv_l4_weights,beta_l4,gamma_l4)
