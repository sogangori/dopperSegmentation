...
modelName = "./IQ_COLOR/weights/bn_self.pd"
LABEL_SIZE_C = 2
NUM_CHANNELS_In= 3
...
with tf.variable_scope('bimap'):
    ...
    beta_xx0 = tf.Variable(tf.constant(0.0, shape=[depth0]))
    gamma_xx0 = tf.Variable(tf.constant(1.0, shape=[depth0])) 

    conv_x2_weights = tf.get_variable("x2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_x2 = tf.Variable(tf.constant(0.0, shape=[depth0]))
    gamma_x2 = tf.Variable(tf.constant(1.0, shape=[depth0])) 

    conv_p2_weights = tf.get_variable("p2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_p2 = tf.Variable(tf.constant(0.0, shape=[depth0]))
    gamma_p2 = tf.Variable(tf.constant(1.0, shape=[depth0]))

    conv_t2_weights = tf.get_variable("t2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_t2 = tf.Variable(tf.constant(0.0, shape=[depth0]))
    gamma_t2 = tf.Variable(tf.constant(1.0, shape=[depth0]))

    conv_s2_weights = tf.get_variable("s2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_s2 = tf.Variable(tf.constant(0.0, shape=[depth0]))
    gamma_s2 = tf.Variable(tf.constant(1.0, shape=[depth0]))

    conv_m2_weights = tf.get_variable("m2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
    beta_m2 = tf.Variable(tf.constant(0.0, shape=[depth0]),name='beta_m2')
    gamma_m2 = tf.Variable(tf.constant(1.0, shape=[depth0]),name='gamma_m2') 
...
def inference(inData, train,step):
    ...
    pool = helper.Gaussian_noise_Add(pool, 0.2, 0.5)
    ...
    #1/8
    feature2 = pool = helper.conv2dBN_Relu(pool,conv_s0_weights,beta_s0,gamma_s0)
