...
def main(argv=None):        
  ...
  train_prediction2 = helper.inference2(train_data, train_prediction, True)  
  eval_prediction = 0
          
  # Add the regularization term to the loss.
  loss = mserror = getLoss( train_prediction, train_labels_node)
  loss2 = getLoss( train_prediction2, train_labels_node)
  ...
  optimizer2 = tf.train.MomentumOptimizer(learning_rate, 0.5).minimize(loss2, global_step=batch)
  ...
  with tf.Session(config=config) as sess:    
    ...
    for step in xrange(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
      ...
      _,_2, mse, l,l2, lr, predictions = sess.run(
          [optimizer,optimizer2, mserror, loss,loss2, learning_rate, train_prediction], feed_dict=feed_dict)
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        print('Step %d (epoch %.1f), %.0f ms, loss %f,loss 2%f lr %f' % 
...
def getLoss( prediction,labels_node):    
    return tf.reduce_mean(tf.square( labels_node - prediction))
