...
modelName = "./DAS/weights/skip3.pd"
logName ="./DAS/weights/logs_skip3"
...
conv_s2_weights = tf.Variable(tf.truncated_normal([3, 3, depth2+depth2, depth2],stddev=0.1,seed=SEED))
...
conv_m2_weights = tf.Variable(tf.truncated_normal([3, 3, depth1+depth1, depth1],stddev=0.1,seed=SEED))
...
conv_l2_weights = tf.Variable(tf.truncated_normal([3, 3, depth0+depth0, depth2],stddev=0.1,seed=SEED))
...
def inference(inData, train=False):
    ...
    #1/2
    featureO = tf.multiply(inData ,1.0)
    ...
    pool = conv2dRelu(pool,conv_roi1_weights,conv_roi1_biases,isDrop)
    ...
    pool = conv2dRelu(pool,conv_s3_weights,conv_s3_biases,isDrop)
    ...
    pool = conv2dRelu(pool,conv_m3_weights,conv_m3_biases,isDrop)
