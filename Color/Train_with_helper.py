...
BATCH_SIZE = numpy.int(DATA_SIZE / 2)  # * AUGMENT
NUM_EPOCHS = 20
isNewTrain = not True                 
...
def main(argv=None):        
  ...
  train_help_node = tf.placeholder(tf.float32, shape=batch_train_out_shape)
  ...
  entropy = getLoss(train_prediction, train_labels_node,train_help_node)
  #entropy = getLoss_morph(train_prediction, train_labels_node,train_help_node,filter)
  ...
  with tf.Session(config=config) as sess:    
    ...
    for step in xrange(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        print('%d/%.1f, %.0f ms, loss %f, acc %.1f, %.2f, lr %.4f, %s' % 
...
def getLoss(prediction,labels_node,help):    
    ...
    help_reshape = tf.reshape(help, [-1])
    ...
    labelResionError = (entropy * help_reshape)*0.05
    return  tf.reduce_mean(entropy+labelResionError)    

def getLoss_morph(prediction,labels_node,help,filter):
    
    prediction = tf.nn.dilation2d(prediction, filter,[1,1,1,1],rates=[1,1,1,1], padding="SAME")
    print ('------------------dil',prediction._shape)
    prediction =  tf.reshape(prediction, [-1, model.LABEL_SIZE_C])
    shape = labels_node.get_shape().as_list()
    label_reshape = tf.reshape(labels_node, [-1])
    help_reshape = tf.reshape(help, [-1])
    ...
    #and labels of shape [batch_size]. But higher dimensions are supported.
    entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = prediction, labels = label_reshape)
    labelResionError = (entropy * help_reshape)*0.0001
    return  tf.reduce_mean(entropy+labelResionError)
