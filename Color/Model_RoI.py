...
conv_roi_0_weights = tf.get_variable("roi0", shape=[3, 3, NUM_CHANNELS_In, depth0], initializer =tf.contrib.layers.xavier_initializer())
conv_roi_0_biases = tf.Variable(tf.zeros([depth0]))
conv_roi_1_weights = tf.get_variable("roi1", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
conv_roi_1_biases = tf.Variable(tf.zeros([depth0]))
conv_roi_2_weights = tf.get_variable("roi2", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
conv_roi_2_biases = tf.Variable(tf.zeros([depth0]))
conv_roi_3_weights = tf.get_variable("roi3", shape=[3, 3, depth0, depth0], initializer =tf.contrib.layers.xavier_initializer())
conv_roi_3_biases = tf.Variable(tf.zeros([depth0]))
...
conv_l0_weights  = tf.get_variable("w1", shape=[3, 3, NUM_CHANNELS_In+LABEL_SIZE_C, depth0], initializer =tf.contrib.layers.xavier_initializer())
...
def inference(inData, train=False):
    ...
    roi = helper.conv2dRelu(in2,conv_roi_0_weights,conv_roi_0_biases)
    roi = tf.nn.max_pool(roi,pool_stride2,strides=pool_stride2,padding='SAME')
    roi = helper.conv2dRelu(roi,conv_roi_1_weights,conv_roi_1_biases)
    roi = tf.nn.max_pool(roi,pool_stride2,strides=pool_stride2,padding='SAME')
    roi = helper.conv2dRelu(roi,conv_roi_2_weights,conv_roi_2_biases)
    roi = tf.nn.max_pool(roi,pool_stride2,strides=pool_stride2,padding='SAME')
    roi = helper.conv2dRelu(roi,conv_roi_3_weights,conv_roi_3_biases)
    roi = tf.nn.max_pool(roi,pool_stride2,strides=pool_stride2,padding='SAME')
    roi = helper.conv2dRelu(roi,conv_roi_4_weights,conv_roi_4_biases)    
    ...
    in_roi = tf.concat([in2, roi_re ],3)
    feature1 = pool = helper.conv2dRelu(in_roi,conv_l0_weights,conv_l0_biases)
