...
modelName = "./Color/weights/depthwise3goLast.pd"
...
conv_o0_weights  = tf.get_variable("o0", shape=[3, 3, NUM_CHANNELS_In, 1], initializer =tf.contrib.layers.xavier_initializer())
conv_o0_biases = tf.Variable(tf.zeros([NUM_CHANNELS_In]))

conv_l0_weights  = tf.get_variable("l1", shape=[3, 3, NUM_CHANNELS_In, depth0], initializer =tf.contrib.layers.xavier_initializer())
...
def inference(inData, train=False):
    ...
    in2  = helper.depthwiseConv2dRelu(inData,conv_o0_weights,conv_o0_biases)   
    in2 = tf.nn.max_pool(in2,pool_stride2,strides=pool_stride2,padding='SAME')    
    #in2 = inData 
    ...
    pool = helper.upConvRelu(pool,dconv_final_weights,dconv_final_biases, inData.get_shape().as_list())    
    pool = helper.depthwiseConv2dRelu(pool,conv_o2_weights,conv_o2_biases)
