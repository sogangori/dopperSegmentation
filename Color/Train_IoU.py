...
import Model_bn_IoU_narrow as model 
...
def main(argv=None):        
  ...
  argMax = tf.cast( tf.round(prediction),tf.int32)    
  mean_iou = getIoU(Y,argMax)  
  loss_iou = 1 - mean_iou 
  loss = loss_iou+ 1e-6 * regullarizer()    
  batch = tf.Variable(0)
  LearningRate = 0.001
  DecayRate = 0.999
  ...
  with tf.Session(config=config) as sess:    
    ...
    for step in xrange(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
      ...
      _, l2, iou,lr = sess.run([optimizer, loss_iou, mean_iou, learning_rate], feed_dict)
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        print('%d/%.1f, %.0f ms, loss(%.3f),IoU(%g,%.3f),lr %.4f, %s' % 
              (step, float(step) * BATCH_SIZE / train_size,takes,l2,iou,iou_test, lr*100,now))   
    ...
    DataReader.SaveAsImage(predict[:,:,:,0], predictImagePath, EVAL_BATCH_SIZE, maxCount = 10)
